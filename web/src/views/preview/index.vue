...
import {NResult, NMarquee, NImage, NButton, NDrawerContent, NTag, NDrawer, NModal, NCard} from 'naive-ui'
...
import {ProjectType, useProjectStore} from "@/stores/useProject.ts";
import {SERVE_URL} from "../../../utils";
...
const handleSceneMark = async (nextSceneId: string) => {
  try {
    ...
    await data.PSV?.setPanorama(SERVE_URL + nextViewer.sceneImg, {
      transition: false,
      position: {
...
onMounted(async () => {
  try {
    loading.value = true
    const {id, local} = route.query
    if (local) {
      projectInfo.value = useStorage('project-list', []).value.find(o => o.id === Number(id))
    } else {
      const res = await projectStore.getProjectByIdAsync(Number(id))
      projectInfo.value = res.data
    }
    if (!projectInfo.value) return
    const hasViewer = !!projectInfo.value.viewerList.length
    if (!hasViewer) return
    currViewerId.value = projectInfo.value.viewerList[0]?.id
    await nextTick()
    await initViewer()
  } catch (e) {
    console.log(e, '???')
    ...
    <n-modal :z-index="99999999999" to="#viewer" v-model:show="pop.show" preset="dialog" closable
      ...
      <template #default v-if="pop.info">
        <div v-if="pop.type === MarkClassType.IMG">
          ...
          <n-image :src="SERVE_URL + pop.info.image" style="max-height: 500px" :preview-disabled="true" object-fit="fill"></n-image>
    ...
    <div class="viewer-list">
      <template v-for="viewer in projectInfo.viewerList" :key="viewer.id">
        <div class="viewer-item relative" :class="currViewerId === viewer.id ? 'active':''"
          ...
          <n-image :src="SERVE_URL + viewer.sceneImg" width="180px" :preview-disabled="true" object-fit="fill"></n-image>
